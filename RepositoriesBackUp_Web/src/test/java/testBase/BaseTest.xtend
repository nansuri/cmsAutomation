...
class BaseTest {
	...
	private String winScreenShotFileLoc = windowsPath + "\\" + getClass.simpleName + "\\"
	...
	@BeforeMethod
	@Parameters("browser")
	def public void BeforeTest(@Optional(ConfigKeys.BROWSER_TYPE)String browser) throws MalformedURLException {

		try {
			if (browser.equals("CHROME")) {

				try {
					driver = new RemoteWebDriver(new URL(ConfigKeys.CHROME_NODEURL),optionsManager.chromeOptions);
				...
				try {
					driver = new RemoteWebDriver(new URL(ConfigKeys.FF_NODEURL), optionsManager.firefoxOptions);
				} catch (MalformedURLException e) {
					e.printStackTrace();
				...
				try {
					driver = new RemoteWebDriver(new URL(ConfigKeys.IE_NODEURL), optionsManager.MSEdgeOptions);
				} catch (MalformedURLException e) {
					e.printStackTrace();
